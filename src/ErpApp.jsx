...
import { G, GL, SF, S, baseInp, dlXlsx, sC, pC, nowT, ThemeCtx } from "./constants.js";
...
function ErpApp() {
  ...
  useEffect(()=>{
    ...
    document.body.style.color = T.cream;
  },[theme]);
    ...
    <div className="erp-root" style={{fontFamily:S,background:T.bg,minHeight:"100vh",color:T.cream,maxWidth:600,margin:"0 auto",zoom:zoomLevel}}>
      ...
      <style>{`
        .erp-root {
          ...
          --border: ${T.border};
          --copper: ${T.copper};
          --copperLight: ${T.copperLight};
          --copperGlow: ${T.copperGlow};
          --cream: ${T.cream};
          --muted: ${T.creamMuted};
          --green: ${T.green};
          --red: ${T.red};
        ...
        .erp-root input[type="checkbox"] { accent-color: ${T.copper}; }
      ...
      <div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,padding:"10px 14px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:10}}>
        ...
        <div style={{display:"flex",gap:4,alignItems:"center"}}>
          {[{l:"주문",v:orders.length,c:T.copper},{l:"미입금",v:unpaid,c:unpaid>0?T.red:T.creamMuted},{l:"부족",v:low.length,c:low.length>0?T.yellow:T.creamMuted}].map(s=>(
      ...
      <div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,display:"flex",position:"sticky",top:50,zIndex:9,overflowX:"auto",WebkitOverflowScrolling:"touch"}}>
        ...
        {TABS.map((t,i)=>(
          <button key={i} onClick={()=>setTab(i)} style={{
            ...
            borderBottom:tab===i?`2px solid ${T.copper}`:"2px solid transparent",
            background:tab===i?T.copperGlow:"transparent",
            color:tab===i?T.copper:T.creamMuted,
      ...
      <div style={{padding:16}}>
          ...
          <div style={{paddingBottom: selectMode && selectedOrders.length>0 ? 80 : 0}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
              ...
              <div style={{display:"flex",gap:6,alignItems:"center"}}>
                <button onClick={()=>{setSelectMode(s=>!s);setSelectedOrders([]);}} style={{padding:"6px 12px",borderRadius:8,border:`1.5px solid ${selectMode?T.copper:T.border}`,background:selectMode?T.copperGlow:"transparent",color:selectMode?T.copper:T.creamMuted,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:S}}>
                  {selectMode?"✕ 선택취소":"☑ 선택"}
                </button>
                <GhostBtn onClick={()=>setShippingOpen(true)} small color={T.copper}>🚚 전체송장</GhostBtn>
            ...
            {/* 필터 토글 */}
            <div style={{marginBottom:10}}>
              <button onClick={()=>setShowFilter(f=>!f)} style={{fontSize:12,color:showFilter?T.copper:T.creamMuted,background:showFilter?T.copperGlow:"transparent",border:`1px solid ${showFilter?T.copper:T.border}`,borderRadius:8,padding:"6px 12px",cursor:"pointer",fontFamily:S,fontWeight:600}}>
              ...
              <Card style={{marginBottom:12,background:T.card,border:`1px solid ${T.border}`}}>
                ...
                <div style={{marginBottom:10}}>
                  ...
                  <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
                    {["",  ...managers].map(m=>(
                      <button key={m} onClick={()=>setOrderFilter(f=>({...f,manager:m}))} style={{padding:"4px 10px",fontSize:11,borderRadius:16,border:`1px solid ${orderFilter.manager===m?T.copper:T.border}`,background:orderFilter.manager===m?T.copperGlow:"transparent",color:orderFilter.manager===m?T.copper:T.creamMuted,cursor:"pointer",fontFamily:S,fontWeight:orderFilter.manager===m?700:400}}>
                        {m||"전체"}
                      </button>
                    ))}
                  </div>
                ...
                <div style={{marginBottom:10}}>
                  ...
                  <div style={{display:"flex",gap:5}}>
                    {["","접수","준비중","출고완료"].map(s=>(
                      <button key={s} onClick={()=>setOrderFilter(f=>({...f,status:s}))} style={{padding:"4px 10px",fontSize:11,borderRadius:16,border:`1px solid ${orderFilter.status===s?T.copper:T.border}`,background:orderFilter.status===s?T.copperGlow:"transparent",color:orderFilter.status===s?T.copper:T.creamMuted,cursor:"pointer",fontFamily:S,fontWeight:orderFilter.status===s?700:400}}>
                        {s||"전체"}
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div style={{display:"flex",gap:5}}>
                    {["","입금완료","미입금"].map(p=>(
                      <button key={p} onClick={()=>setOrderFilter(f=>({...f,payment:p}))} style={{padding:"4px 10px",fontSize:11,borderRadius:16,border:`1px solid ${orderFilter.payment===p?T.copper:T.border}`,background:orderFilter.payment===p?T.copperGlow:"transparent",color:orderFilter.payment===p?T.copper:T.creamMuted,cursor:"pointer",fontFamily:S,fontWeight:orderFilter.payment===p?700:400}}>
                        {p||"전체"}
                      </button>
                    ))}
                  </div>
            ...
            {/* 전체선택 바 */}
            {selectMode && (
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 14px",background:T.copperGlow,borderRadius:10,border:`1px solid ${T.copper}40`,marginBottom:12}}>
                <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",fontSize:13,fontWeight:600,color:T.copper}}>
                  <input type="checkbox"
                    checked={selectedOrders.length===orders.length && orders.length>0}
                    onChange={e=>setSelectedOrders(e.target.checked?orders.map(o=>o.id):[])}
                    style={{accentColor:T.copper,width:16,height:16}}
                  />
                  전체 선택 ({selectedOrders.length}/{orders.length})
                </label>
                {selectedOrders.length>0 && (
                  <button onClick={()=>{setShippingOpen(true);}} style={{padding:"6px 14px",borderRadius:8,border:"none",background:`linear-gradient(135deg,${T.copper},${T.copperLight})`,color:T.white,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:S,boxShadow:`0 2px 12px ${T.copperGlow}`}}>
            ...
            {(()=>{
              ...
              : filtered.map(o=>{
                  ...
                  return (
                    <Card key={o.id} style={{marginBottom:10,border:`1.5px solid ${isSelected?T.copper:T.border}`,background:isSelected?T.copperGlow:T.card,transition:"all 0.15s"}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
                        <div style={{display:"flex",alignItems:"flex-start",gap:10,flex:1}}>
                          {selectMode && (
                            <input type="checkbox" checked={isSelected}
                              onChange={e=>setSelectedOrders(p=>e.target.checked?[...p,o.id]:p.filter(x=>x!==o.id))}
                              style={{accentColor:T.copper,width:18,height:18,marginTop:2,flexShrink:0,cursor:"pointer"}}
                            />
                          )}
                          <div style={{flex:1}}>
                            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4,flexWrap:"wrap"}}>
                              <span style={{fontFamily:SF,fontSize:11,color:T.copper,fontWeight:700}}>{o.id}</span>
                              <span style={{fontWeight:800,fontSize:16}}>{o.customer}</span>
                              {o.manager && <span style={{fontSize:10,color:T.creamMuted,background:T.surface,padding:"1px 6px",borderRadius:4}}>{o.manager}</span>}
                            </div>
                            <span style={{fontSize:11,color:T.creamMuted}}>{o.date}{(o.phone || customers.find(x=>x.name===o.customer)?.phone) ? ` · ${o.phone || customers.find(x=>x.name===o.customer)?.phone}` : ""}</span>
                          </div>
                        </div>
                        <div style={{display:"flex",flexDirection:"column",gap:4,alignItems:"flex-end",flexShrink:0}}>
                          <Tag c={pC(o.payment,T)[0]} bg={pC(o.payment,T)[1]}>{o.payment}</Tag>
                          <Tag c={sC(o.status,T)[0]} bg={sC(o.status,T)[1]}>{o.status}</Tag>
                        </div>
                      </div>
                      <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:10}}>
                        {o.items.map((item,i)=>(
                          <div key={i} style={{padding:"4px 10px",background:T.surface,borderRadius:8,border:`1px solid ${T.border}`,fontSize:12}}>
                            <span style={{color:T.creamMuted}}>{item.fabric}{item.color?" "+item.color:""} </span>
                            <span style={{fontWeight:700}}>{item.qty}마</span>
                            {item.amount>0 && <span style={{color:T.copper,marginLeft:4,fontSize:11}}>{item.amount.toLocaleString()}원</span>}
                          </div>
                        ))}
                      </div>
                      {o.address && <div style={{fontSize:11,color:T.creamMuted,marginBottom:6}}>📦 {o.address}{o.addressDetail ? ` ${o.addressDetail}` : ""}</div>}
                      {(o.links||[]).length>0 && <div style={{fontSize:11,marginBottom:6,display:"flex",flexDirection:"column",gap:3}}>{o.links.map((lnk,i)=><a key={i} href={lnk} target="_blank" rel="noopener noreferrer" style={{color:T.blue,textDecoration:"underline"}}>🔗 {lnk.length>50?lnk.slice(0,50)+"...":lnk}</a>)}</div>}
                      {o.note && o.note.trim() && o.note.trim()!=="None" && (
                        <div style={{fontSize:12,color:T.cream,marginBottom:10,padding:"6px 10px",background:T.surface,borderRadius:7,border:`1px solid ${T.border}`,display:"flex",gap:6,alignItems:"flex-start"}}>
                          <span style={{color:T.yellow,fontSize:13,flexShrink:0}}>💬</span>
                        ...
                        <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:8}}>
                          {["접수","준비중","출고완료"].map(s=>{
                            ...
                            return (
                              ...
                              }} style={{padding:"4px 10px",fontSize:11,borderRadius:20,border:`1px solid ${o.status===s?c:T.border}`,background:o.status===s?bg:"transparent",color:o.status===s?c:T.creamMuted,cursor:"pointer",fontFamily:S,fontWeight:o.status===s?700:400}}>
                                {s}
                              </button>
                            );
                          })}
                          {o.payment!=="입금완료" && (
                            <button onClick={()=>{db.updateOrder(o.id,{payment:"입금완료"});setOrders(p=>p.map(x=>x.id===o.id?{...x,payment:"입금완료"}:x));kakaoAlert(`✅ 입금확인\n${o.id} — ${o.customer}`);showToast("입금 확인 완료");}} style={{marginLeft:"auto",padding:"4px 12px",fontSize:11,borderRadius:20,border:`1px solid ${T.green}`,background:T.greenBg,color:T.green,cursor:"pointer",fontFamily:S,fontWeight:700}}>
                              입금확인 ✓
                            </button>
                          )}
                        </div>
                        <div style={{display:"flex",gap:6,paddingTop:8,borderTop:`1px solid ${T.border}`}}>
                          <button onClick={()=>setEditingOrder({...o,items:o.items.map(i=>({...i}))})} style={{flex:1,padding:"7px",borderRadius:8,border:`1px solid ${T.border}`,background:"transparent",color:T.creamMuted,fontSize:12,cursor:"pointer",fontFamily:S}}>✏ 수정</button>
                          <button onClick={()=>setConfirmDel({type:"order",id:o.id,label:`주문 ${o.id} (${o.customer})`})} style={{flex:1,padding:"7px",borderRadius:8,border:`1px solid ${T.red}40`,background:T.redBg,color:T.red,fontSize:12,cursor:"pointer",fontFamily:S}}>🗑 삭제</button>
                        </div>
                      </>}
                      {selectMode && (
                        <div onClick={()=>setSelectedOrders(p=>isSelected?p.filter(x=>x!==o.id):[...p,o.id])}
                          style={{paddingTop:8,borderTop:`1px solid ${T.border}40`,textAlign:"center",fontSize:12,color:isSelected?T.copper:T.creamMuted,cursor:"pointer",fontWeight:isSelected?700:400}}>
            ...
            {selectMode && selectedOrders.length>0 && (
              <div style={{position:"fixed",bottom:20,left:"50%",transform:"translateX(-50%)",zIndex:50,maxWidth:560,width:"calc(100% - 32px)"}}>
                <button onClick={()=>setShippingOpen(true)} style={{width:"100%",padding:"15px",borderRadius:14,border:"none",background:`linear-gradient(135deg,${T.copper},${T.copperLight})`,color:T.white,fontWeight:800,fontSize:16,cursor:"pointer",fontFamily:S,boxShadow:`0 6px 30px ${T.copperGlow}`,display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
                  <span>🚚</span>
        ...
        {tab===2 && (
          <div>
            ...
            {/* 서브 탭 */}
            <div style={{display:"flex",gap:0,background:T.surface,borderRadius:12,padding:3,marginBottom:16}}>
              {[{id:"in",icon:"📥",label:"입고"},{id:"auto",icon:"🔄",label:"자동출고"},{id:"barcode",icon:"▐▌",label:"바코드"},{id:"stock",icon:"📦",label:"재고현황"},{id:"history",icon:"📋",label:"내역"}].map(t=>(
                <button key={t.id} onClick={()=>setIoTab(t.id)} style={{
                  ...
                  background:ioTab===t.id?`linear-gradient(135deg,${T.copper},${T.copperLight})`:"transparent",
                  color:ioTab===t.id?T.white:T.creamMuted,
            ...
            {ioTab==="barcode" && (
              <Card style={{padding:"40px 20px",textAlign:"center",background:T.card,border:`1px solid ${T.border}`}}>
                ...
                <div style={{fontSize:15,fontWeight:700,color:T.cream,marginBottom:6}}>서비스 준비중</div>
                <div style={{fontSize:12,color:T.creamMuted}}>바코드 기능은 현재 준비중입니다.</div>
              </Card>
            )}
              ...
              <div>
                <Card style={{marginBottom:10}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                    ...
                    }} style={{fontSize:11,color:T.yellow,background:T.yellowBg,border:`1px solid ${T.yellow}40`,borderRadius:6,padding:"4px 10px",cursor:"pointer",fontFamily:S,fontWeight:600}}>
                      ⚠ 부족확인
                    </button>
                  </div>
                  {inv.length===0
                    ...
                    : inv.map((i,idx)=>(
                      <div key={i.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"11px 0",borderTop:idx>0?`1px solid ${T.border}`:"none"}}>
                        <div>
                          <div style={{fontSize:14,fontWeight:700}}>{i.fabric}</div>
                          {i.color && <div style={{fontSize:11,color:T.creamMuted,marginTop:1}}>{i.color}</div>}
                        </div>
                        <div style={{display:"flex",alignItems:"center",gap:8}}>
                          <div style={{textAlign:"right"}}>
                            <span style={{fontFamily:SF,fontSize:20,fontWeight:800,color:i.stock<settings.lowStockAlert?T.red:i.stock<30?T.yellow:T.green}}>{i.stock}</span>
                            <span style={{fontSize:11,color:T.creamMuted,marginLeft:2}}>마{i.stock<settings.lowStockAlert?" ⚠":""}</span>
                          </div>
                          <button onClick={()=>setEditingInv({...i})} style={{padding:"4px 9px",borderRadius:7,border:`1px solid ${T.border}`,background:"transparent",color:T.creamMuted,fontSize:12,cursor:"pointer",fontFamily:S}}>✏</button>
                          <button onClick={()=>setConfirmDel({type:"inv",id:i.id,label:`${i.fabric} ${i.color}`})} style={{padding:"4px 9px",borderRadius:7,border:`1px solid ${T.red}40`,background:T.redBg,color:T.red,fontSize:12,cursor:"pointer",fontFamily:S}}>🗑</button>
                        </div>
                      </div>
                    ))
                  }
                  ...
                  {inv.slice(0,12).map(i=>(
                    <div key={i.id} style={{marginBottom:10}}>
                      <div style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:4}}>
                        ...
                        <span style={{fontWeight:700,color:i.stock<settings.lowStockAlert?T.red:i.stock<30?T.yellow:T.green}}>{i.stock}마</span>
                      </div>
                      <div style={{height:6,background:T.trackBg,borderRadius:3,overflow:"hidden"}}>
                        <div style={{height:"100%",width:`${Math.min(100,(i.stock/Math.max(...inv.map(x=>x.stock),1))*100)}%`,background:i.stock<settings.lowStockAlert?T.red:i.stock<30?T.yellow:T.copper,borderRadius:3,transition:"width 0.5s"}}/>
                      </div>
                    </div>
                  ))}
                </Card>
              ...
              <div>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                  ...
                  <div style={{display:"flex",gap:6}}>
                    <Tag c={T.green} bg={T.greenBg}>입고 {logs.filter(l=>l.type==="입고").length}</Tag>
                    <Tag c={T.copper} bg={T.copperGlow}>출고 {logs.filter(l=>l.type==="출고").length}</Tag>
                  </div>
                </div>
                {logs.length===0
                  ? <Empty text="입출고 내역이 없습니다" sub=""/>
                  : logs.map(log=>(
                    <div key={log.id} style={{display:"flex",alignItems:"center",gap:10,padding:"12px 14px",background:T.card,borderRadius:10,border:`1px solid ${T.border}`,marginBottom:6}}>
                      <Tag c={log.type==="입고"?T.green:T.copper} bg={log.type==="입고"?T.greenBg:T.copperGlow}>{log.type}</Tag>
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{fontSize:14,fontWeight:700}}>{log.fabric}{log.color?" · "+log.color:""}</div>
                        <div style={{fontSize:11,color:T.creamMuted,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
                      {log.date} · {log.ref}
                      {log.costPrice>0&&<span style={{color:T.copper,marginLeft:4}}>@{log.costPrice.toLocaleString()}원</span>}
                      {log.note&&log.note!=="—"?<span> · {log.note}</span>:""}
                    </div>
                      </div>
                      <span style={{fontFamily:SF,fontSize:18,fontWeight:800,color:log.type==="입고"?T.green:T.copper}}>{log.qty}<span style={{fontSize:10,color:T.creamMuted,marginLeft:2}}>마</span></span>
                      <button onClick={()=>setConfirmDel({type:"log",id:log.id,label:`${log.type} ${log.fabric} ${log.qty}마`})} style={{padding:"4px 7px",borderRadius:6,border:`1px solid ${T.red}40`,background:T.redBg,color:T.red,fontSize:11,cursor:"pointer",fontFamily:S,flexShrink:0}}>🗑</button>
                    </div>
                  ))
                }
              </div>
        ...
        {tab===5 && (
          <div>
            <div style={{fontFamily:SF,fontSize:22,fontWeight:800,marginBottom:16,color:T.cream}}>대시보드</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
              {[{icon:"◈",l:"총 주문",v:orders.length,u:"건",c:T.copper},{icon:"◉",l:"총 수량",v:totalQty,u:"마",c:T.blue},{icon:"◇",l:"미입금",v:unpaid,u:"건",c:unpaid>0?T.red:T.creamMuted},{icon:"♦",l:"고객수",v:customers.length,u:"명",c:T.purple}].map(s=>(
                <div key={s.l} style={{background:T.card,borderRadius:14,border:`1px solid ${T.border}`,padding:"16px 18px",position:"relative",overflow:"hidden"}}>
                  ...
                  <div style={{fontFamily:SF,fontSize:28,fontWeight:700,color:s.c,lineHeight:1}}>{s.v}<span style={{fontSize:12,fontWeight:400,color:T.creamMuted,marginLeft:3}}>{s.u}</span></div>
                  <div style={{fontSize:11,color:T.creamMuted,marginTop:5}}>{s.l}</div>
                </div>
              ))}
            </div>
            <Card style={{marginBottom:12}}>
              ...
              {orders.length===0
                ? <div style={{color:T.creamMuted,fontSize:13,textAlign:"center",padding:"16px 0"}}>주문 없음</div>
                : orders.slice(0,5).map((o,i)=>(
                  <div key={o.id} style={{display:"flex",alignItems:"center",gap:8,padding:"9px 0",borderTop:i>0?`1px solid ${T.border}`:"none"}}>
                    <span style={{fontFamily:SF,fontSize:11,color:T.copper,fontWeight:700,minWidth:44}}>{o.id}</span>
                    <span style={{fontWeight:600,flex:1,fontSize:13}}>{o.customer}</span>
                    <span style={{fontSize:11,color:T.creamMuted}}>{o.items.reduce((a,i)=>a+i.qty,0)}마</span>
                    <Tag c={sC(o.status,T)[0]} bg={sC(o.status,T)[1]}>{o.status}</Tag>
                  </div>
                ))
              }
              ...
              {inv.map(i=>(
                <div key={i.id} style={{marginBottom:12}}>
                  <div style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:5}}>
                    <span style={{fontWeight:600}}>{i.fabric} <span style={{color:T.creamMuted,fontWeight:400}}>{i.color}</span></span>
                    <span style={{fontWeight:700,color:i.stock<settings.lowStockAlert?T.red:i.stock<30?T.yellow:T.green}}>{i.stock}마</span>
                  </div>
                  <div style={{height:4,background:T.trackBg,borderRadius:2,overflow:"hidden"}}>
                    <div style={{height:"100%",width:`${Math.min(100,(i.stock/100)*100)}%`,background:i.stock<settings.lowStockAlert?T.red:i.stock<30?T.yellow:T.copper,borderRadius:2,transition:"width 0.5s"}}/>
                  </div>
                </div>
              ))}
            </Card>
        ...
        {tab===6 && (
          <div>
            ...
            <Card style={{marginBottom:14,background:T.card,border:`1px solid ${T.border}`}}>
              ...
              <div style={{fontSize:12,color:T.creamMuted,marginBottom:10,lineHeight:1.7}}>
                더브릿지 서비스 소개 · 요금제 · 데모 체험
              </div>
              <button onClick={()=>{
                window.open("https://claude.site/artifacts/8e57d5de-2c70-4783-b7a9-fd0fe8a05e49","_blank");
              }} style={{width:"100%",padding:13,borderRadius:10,border:`1.5px solid ${T.copper}`,
                background:T.copperGlow,color:T.copper,fontWeight:800,fontSize:14,
              ...
              }} style={{width:"100%",padding:11,borderRadius:10,border:`1px solid ${T.border}`,
                background:"transparent",color:T.creamMuted,fontWeight:700,fontSize:13,
              ...
              <div style={{marginBottom:10}}>
                ...
                {settings.anthropicKey ? (
                  <div style={{display:"flex",gap:8,alignItems:"center"}}>
                    ...
                    <GhostBtn small color={T.red} onClick={()=>setSettings(s=>({...s,anthropicKey:""}))}>초기화</GhostBtn>
                  </div>
                ) : (
                  <input type="password" value="" onChange={e=>setSettings(s=>({...s,anthropicKey:e.target.value}))} placeholder="sk-ant-..." style={baseInp}/>
                )}
              </div>
              <div style={{fontSize:11,color:T.creamMuted,lineHeight:1.5}}>AI 주문 분석 기능에 사용됩니다. 키는 브라우저에만 저장됩니다.</div>
            </Card>
            ...
            <Card style={{marginBottom:14,background:T.card,border:`1px solid ${T.border}`}}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                <span style={{fontSize:13,color:T.creamMuted}}>{settings.kakaoEnabled?"활성화":"비활성화"}</span>
                <div style={{width:38,height:22,borderRadius:11,background:settings.kakaoEnabled?T.copper:T.border,position:"relative",cursor:"pointer"}} onClick={()=>setSettings(s=>({...s,kakaoEnabled:!s.kakaoEnabled}))}>
                  <div style={{width:18,height:18,borderRadius:"50%",background:T.white,position:"absolute",top:2,left:settings.kakaoEnabled?18:2,transition:"left 0.2s"}}/>
                </div>
              </div>
